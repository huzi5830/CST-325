...
function updateAndRender() {
    ...
    // todo #10
    var light = new Vector4(lightDirection.x, lightDirection.y, lightDirection.z, 0)
    var rotation = new Matrix4();
    if (appInput.up == true){
      //console.log(light);
      rotation.makeRotationX(-2);
    } 
    else if (appInput.down == true){
      rotation.makeRotationX(2);
    }
    else if (appInput.right == true){
        rotation.makeRotationY(2);
    }
    else if (appInput.left == true){
        rotation.makeRotationY(-2);
    }
    // add keyboard controls for changing light direction here
    var rotatedLight= rotation.multiplyVector(light);
